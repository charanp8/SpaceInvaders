...
// I.e., PortB has already been reset and activated (do not reset PortB here again)
  // write this
    IOMUX->SECCFG.PINCM[PB0INDEX] = 0x00000081; // DACBit0
    IOMUX->SECCFG.PINCM[PB1INDEX] = 0x00000081; // DACBit1
    IOMUX->SECCFG.PINCM[PB2INDEX] = 0x00000081; // DACBit2
    IOMUX->SECCFG.PINCM[PB3INDEX] = 0x00000081; // DACBit3
    IOMUX->SECCFG.PINCM[PB4INDEX] = 0x00000081; // DACBit4

    GPIOB->DOE31_0 |= 0x0000001F; //create mask
}
...
void DAC5_Out(uint32_t data){
    // write this
    uint32_t notsensormask = 0x0000001F;
    GPIOB->DOUT31_0 = ((GPIOB->DOUT31_0 & (~notsensormask)) | data);
}
